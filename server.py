...
# NOTE: we import with relative package paths because uvicorn loads `backend.server:app`
...
def set_auth_cookies(resp: Response, access: str, refresh: str):
    resp.set_cookie("access_token", access, httponly=True, secure=True, samesite="lax", domain=COOKIE_DOMAIN)
    resp.set_cookie("refresh_token", refresh, httponly=True, secure=True, samesite="lax", domain=COOKIE_DOMAIN)
...
def register_doctor(data: RegisterDoctorIn, response: Response, db: Session = Depends(get_db)):
    ...
    u = User(
        ...
        sentinel=SentinelKind.multi.value,
...
def register_patient(data: RegisterPatientIn, response: Response, db: Session = Depends(get_db)):
    existing = db.query(User).filter(User.email == data.email).first()
    if existing:
        raise HTTPException(status_code=400, detail="Email already registered")
...
# optional existing routers (don’t crash if missing)
try:
...
def _require_doctor(u: User): 
    if u.role != "doctor": raise HTTPException(status_code=403, detail="Doctors only")
def _require_patient(u: User):
    if u.role != "patient": raise HTTPException(status_code=403, detail="Patients only")
...
def doctor_dashboard(request: Request, db: Session = Depends(get_db)):
    u = _current_user(request, db); _require_doctor(u)
...
def patient_eldercare_dashboard(request: Request, db: Session = Depends(get_db)):
    u = _current_user(request, db); _require_patient(u)
