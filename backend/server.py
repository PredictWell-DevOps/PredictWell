...
def compute_pitch_score(p: PitchRiskInput, factors: List[str]) -> float:
    ...
    if p.pitch_count>85: factors.append("High pitch count")
    if p.days_rest<2: factors.append("Low rest")
    if p.pain_level>=4: factors.append("Reported pain")
    if p.previous_injury: factors.append("History of injury")
    if p.session_intensity>=8: factors.append("High session intensity")
    return s
...
def compute_elder_score(e: EldercareRiskInput, factors: List[str]) -> float:
    ...
    if e.falls_last_6mo>=1: factors.append("Recent falls")
    if e.dizziness>=4: factors.append("Dizziness")
    if e.meds_changed: factors.append("Medication change")
    if e.steps<3000: factors.append("Low activity")
    if e.sleep_hours<6: factors.append("Low sleep")
    if e.hydration_cups<6: factors.append("Low hydration")
    return s

@app.get("/")
def root(): return {"message":"PredictWell API is running. Visit /docs for Swagger."}
...
def risk(req: RiskRequest):
    score, factors = 0.0, []
    if req.pitch: score += compute_pitch_score(req.pitch, factors)
    if req.elder: score += compute_elder_score(req.elder, factors)
...
def eldercare_checkin(payload: EldercareCheckinIn):
    ...
    cur.execute(
        "INSERT INTO eldercare_checkins (patient_id,sleep_hours,hydration_cups,steps,dizziness,mood,notes) VALUES (?,?,?,?,?,?,?)",
        (payload.patient_id,payload.sleep_hours,payload.hydration_cups,payload.steps,payload.dizziness,payload.mood,payload.notes)
    )
